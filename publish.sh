...
# 0. hardlink mirror (same fs as the vault); normalize frontmatter (vault never modified)
...
# 1. export, self-healing: drop any note obsidian-export can't parse and remember it
...
# 3. split markdown/attachments + post-process callouts & sections
...
# 4. build into .build, then sync into the served public/ (keeps public/ stable during the build)
"$HUGO" --minify --cleanDestinationDir --destination "$SITE/.build"
rsync -a --delete "$SITE/.build/" "$SITE/public/"
echo "PUBLISHED: $(find "$SITE/public" -name '*.html' | wc -l) html pages, $(du -sh "$SITE/public" | cut -f1); skiplist=$([ -f "$SKIPLIST" ] && wc -l < "$SKIPLIST" | tr -d " " || echo 0)"
